...
def inference(categories,input_str, input_image, bbox_img, follow_up=False, generate=False):
    ...
    cumulative_prompt=""
    max_categories=[]
    for bbox_index in range(1,6):
        
        print("PROMPT:",prompt)
        if bbox_index==1:
            cumulative_prompt=prompt+"obj1: "
        else:
            cumulative_prompt=cumulative_prompt+f", obj{bbox_index}:"
        input_ids = tokenizer_image_token(cumulative_prompt, tokenizer, return_tensors="pt")
        token_count = input_ids.numel()  # numel gives the total number of elements in the tensor
        #print(f"Number of tokens in cumulative_prompt: {token_count}")
        ...
        prob_tensor=initial_probs[0]
        print("length of prob_tensor", len(prob_tensor))
        category_probs = []
        for category, index in zip(categories, category_indices):
            #print("index:",index)
            prob = prob_tensor[index].item()
        ...
        cumulative_prompt=cumulative_prompt+max_category
        #print("logits shape: ",first_logits_shape)
    ...
    return output_image, markdown_out,max_categories
...
if __name__ == "__main__":
    ...
    for item in data:
        ...
        for obj in item['objects']:
            new_bbox = [int(obj['bndbox']['xmin']), int(obj['bndbox']['ymin']), int(obj['bndbox']['xmax']), int(obj['bndbox']['ymax'])]
            bbox.append(new_bbox)
        output_image, output_text,prediction = inference(categories,input_str, image_path, bbox)
        print("predictions:",prediction)
        if len(prediction) == len(item['objects']):
            for obj, pred in zip(item['objects'], prediction):
                obj['prediction'] = pred
                if pred == obj['name']:
                    correct_predictions += 1
        else:
            print("Warning: The number of predictions does not match the number of objects.")

        total_predictions += len(item['objects'])
